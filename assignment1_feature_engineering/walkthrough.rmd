...
ridge.mod.pred <- predict(ridge.mod, information.gain.validation)
ridge.mod.pred[is.na(ridge.mod.pred)] <- 0

my_data <- as.data.frame(cbind(predicted=(exp(ridge.mod.pred) -1), observed=(exp(information.gain.validation$SalePrice) -1)))
ridge.mod.rmse <- sqrt(mean((ridge.mod.pred - information.gain.validation$SalePrice)^2))
ridge.mod.price_error <- mean(abs((exp(ridge.mod.pred) -1) - (exp(information.gain.validation$SalePrice) -1)))
